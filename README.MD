...
| `orb_level_weights.json` | ❌ Optional | `--orb-level-weights` | Orb type → multiplier for level-gate bonuses (3/6/9). | Uses `DEFAULT_ORB_LEVEL_WEIGHTS`. |
...
The React/Vite app lives in `web/`.
...
Validation and CI-friendly checks:
